...
The default image URL points at a placeholder Docker Hub location; pass
image_url=... or override DEFAULT_IMAGE_URL once you publish under your
own namespace. See Dockerfile in this directory for the build recipe.
"""
...
WORKLOAD_SCRIPTS = (
    WORKLOAD_DIR / "set_spawn.js",
    WORKLOAD_DIR / "walkaround_bot.js",
    WORKLOAD_DIR / "walkaround_worker_bot.js",
)
...
class WalkAround:
    ...
    def deploy(self) -> None:
        def _stage(node: Node, _i: int) -> None:
            ...
            try:
                wd = self.wds[node]
                for src in WORKLOAD_SCRIPTS:
                    local.path(src).copy(machine.path(f"{wd}/{src.name}"))
            finally:
            ...
            try:
                args = (
                    [
                        ...
                        "--no-https",
                        "--compat",
                        "--pwd", "/opt/workload/scripts",
                        "--bind", f"{self.wds[node]}:/opt/workload/scripts",
                    ]
    ...
    def _set_world_spawn(self) -> None:
        ...
        try:
            args = (
                ...
                + self._env_args(0)
                + [self.image_url, "node", "/opt/workload/scripts/set_spawn.js"]
            )
            machine["apptainer"][args]()
        finally:
            if isinstance(machine, SshMachine):
